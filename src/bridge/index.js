...
class Bridge {
  ...
  async start(config, supabase) {
    ...
    if (taskerStatus.available) {
      this.log('INFO', `Tasker Service available - OAGI: ${taskerStatus.oagiAvailable}`);
    } else {
  ...
  async handleNewTask(task) {
    ...
    if (!taskerStatus.available || !taskerStatus.oagiAvailable) {
      this.log('WARN', 'Tasker Service not ready - waiting...');
      return;
    }

    this.currentTask = task;
    this.log('INFO', `New task: ${task.task_description}`);
    this.log('INFO', `Mode: ${task.lux_mode || 'actor'} | Model: ${task.lux_model || 'lux-actor-1'}`);
